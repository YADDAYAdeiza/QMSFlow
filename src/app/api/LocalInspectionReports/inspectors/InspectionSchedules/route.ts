...
//       if (batchError) {
//         console.error('Batch Shell Creation Failed:', batchError);
//       }
...
//     // 🛡️ CRITICAL GUARD: Reject operation if batch container resolution fails
//     if (!assignedBatchId) {
//       return NextResponse.json(
//         { success: false, error: "Batch Binding Failure: Unable to resolve or generate target schedule batch." },
//         { status: 500 }
//       );
//     }
...
export async function POST(request: Request) {
  try {
    const supabase = await createClient();
    const body = await request.json();
    const { applicationId, batchId, scheduledDate, teamLeader, coInspectors, traineeInspectors } = body;

    // Direct validation: Require batchId explicitly
    if (!applicationId || !batchId || !scheduledDate || !teamLeader) {
      return NextResponse.json(
        { success: false, error: "Validation Error: Missing execution parameters or Batch ID." },
        { status: 400 }
      );
    }

    // STEP 1: Wipe existing schedules for rework (Idempotency)
    ...
    if (existing && existing.length > 0) {
      ...
      await supabase.from('inspection_team_assignments').delete().in('schedule_id', scheduleIds);
      await supabase.from('inspection_schedules').delete().eq('application_id', applicationId);
    }

    // STEP 2: Direct insertion with explicit batch_id
    const { data: schedule, error: scheduleError } = await supabase
      .from('inspection_schedules')
      .insert({
        application_id: applicationId,
        scheduled_date: scheduledDate,
        batch_id: batchId, // Explicit relationship
        status: 'SCHEDULED'
      })
      .select('id')
      .single();
    ...
    // STEP 3: Map Team Assignments
    const assignments = [
      { schedule_id: schedule.id, inspector_id: teamLeader, role: 'TEAM_LEADER' }
    ];

    if (Array.isArray(coInspectors)) {
      coInspectors.forEach(id => assignments.push({ schedule_id: schedule.id, inspector_id: id, role: 'CO_INSPECTOR' }));
    }

    if (Array.isArray(traineeInspectors)) {
      if (traineeInspectors.length > 2) {
        return NextResponse.json({ success: false, error: "QMS Guardrail: Max 2 trainees allowed." }, { status: 400 });
      }
      traineeInspectors.forEach(id => assignments.push({ schedule_id: schedule.id, inspector_id: id, role: 'TRAINEE_INSPECTOR' }));
    }

    const { error: bridgeError } = await supabase.from('inspection_team_assignments').insert(assignments);
    if (bridgeError) throw bridgeError;

    return NextResponse.json({ success: true, scheduleId: schedule.id, batchId });
