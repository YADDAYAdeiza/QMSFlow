...
import ReviewSubmissionForm from "@/components/ReviewSubmissionForm";
import { notFound, redirect } from "next/navigation";
import { Lock, ShieldAlert, FileText, Activity, ExternalLink, FileCheck } from "lucide-react"; 
...
export default async function TechnicalReviewPage(props: PageProps) {
  ...
  const currentDivision = (sParams.as || "VMD").toUpperCase();
  ...
  /**
   * PASS LOGIC
   * We check if inspectionReportUrl exists to determine if we've moved to Pass 2.
   */
  ...
  /**
   * URL RESOLUTION
   * For the current JSON provided: poaUrl is the target for Pass 1.
   * Bucket changed to lowercase 'documents'.
   */
  ...
  // Helper to extract path from full URL if stored as absolute URL in DB
  ...
  if (!activeTask) return <div className="h-screen flex items-center justify-center font-black uppercase text-slate-300">Task Inactive</div>;

  const secondsUsed = allSegments.filter(s => s.staffId === assignedUser.id).reduce((acc, s) => (s.startTime && s.endTime) ? acc + Math.floor((s.endTime.getTime() - s.startTime.getTime()) / 1000) : acc, 0);
  const remaining = (48 * 60 * 60) - secondsUsed;
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50">
      <div className="w-2/3 h-full border-r border-slate-200 bg-slate-900 relative">
        <div className="absolute top-6 left-6 z-10 flex gap-3">
            ...
            {secondaryUrl && (
              <a href={secondaryUrl} target="_blank" rel="noreferrer" className="px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase shadow-2xl flex items-center gap-2 border bg-emerald-600 text-white border-emerald-400">
        ...
        {publicUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-slate-500 font-bold uppercase text-[10px]">Reference not found</div>
        )}
      </div>
      ...
      <div className="w-1/3 h-full flex flex-col bg-white overflow-y-auto">
        <div className="p-8 pb-32">
          <ReviewSubmissionForm 
            ...
            isComplianceReview={isComplianceReview}
            riskId={riskData?.id} 
            previousFindings={details.findings_ledger || []}
            previousIsSra={details.is_sra || false}
          />
        </div>
