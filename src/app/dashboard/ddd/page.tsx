...
// @/app/dashboard/ddd/page.tsx
import { db } from "@/db";
import { applications, companies, qmsTimelines } from "@/db/schema";
import { eq, and, or, sql, isNull } from "drizzle-orm";
import DossierLink from "@/components/DossierLink";
import { MessageSquare, ArrowRightCircle, ShieldCheck, Activity, Clock } from "lucide-react";
import Link from "next/link";
...
export default async function DDDInboxPage({ 
  ...
  // 2. Fetch Inbox matching your schema: staffId
  const rawInbox = await db
    ...
    .where(and(
      // FIXED: Using 'staffId' to match your exported schema variable
  ...
  const inbox = rawInbox.map(app => {
    ...
    let displayTime = `${minutes}m`;
    if (days > 0) displayTime = `${days}d ${hours % 24}h`;
    else if (hours > 0) displayTime = `${hours}h ${minutes % 60}m`;
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen font-sans">
      ...
      <header className="mb-8">
        ...
        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">
          {actingDivision === "IRSD" ? "Agency Recommendation Hub" : "Technical Review Queue"}
        </p>
      </header>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-900 text-white">
            <tr>
              ...
              <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-300">Latest Status</th>
              <th className="p-6 text-[10px] font-black uppercase tracking-widest text-right text-slate-300">Action</th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={app.id} className="hover:bg-blue-50/30 transition-colors border-b border-slate-100">
                  ...
                  <td className="p-6">
                    <div className="flex flex-col gap-2 max-w-md">
                       ...
                       <p className="text-[10px] text-slate-600 italic line-clamp-1 border-l-2 border-slate-200 pl-2">
                         {lastComment?.text || "No recent comments."}
                       </p>
                    </div>
                  </td>
                  ...
                  <td className="p-6 text-right">
                    <Link 
                      ...
                      className="inline-flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-full text-[10px] font-black uppercase hover:bg-blue-600 transition-all shadow-md"
                    >
                      Process <ArrowRightCircle className="w-4 h-4" />
                    </Link>
                  </td>
                </tr>
              );
        ...
        {inbox.length === 0 && (
          <div className="p-20 text-center text-slate-400 italic font-medium">
            No dossiers found for this desk.
          </div>
        )}
      </div>
    </div>
