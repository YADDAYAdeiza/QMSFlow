...
export default async function Page({ 
  ...
  const appDetails = (app.details as Record<string, any>) || {};
  const history = appDetails.comments || [];

  /**
   * RESOLUTION LOGIC:
   * 1. Check if a Staff member has uploaded a report in the narrative (latest one wins).
   * 2. Otherwise, fall back to the root inspectionReportUrl.
   * 3. Finally, fall back to poaUrl.
   */
  const latestStaffSubmission = [...history].reverse().find((c: any) => c.reportUrl);
  const activePdfUrl = latestStaffSubmission?.reportUrl || appDetails.inspectionReportUrl || appDetails.poaUrl || "";
