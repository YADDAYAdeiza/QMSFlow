...
// Instantiate outside the component lifecycle to avoid instantiation on every re-render
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
);

// Helper function to extract date cleanly from either Space-separated or ISO string dates
...
export default function RegistrationDashboard({ 
  ...
  console.log('This is companiesCatalog now: ', companiesCatalog);
  ...
  console.log('This is activeLedgerEntry: ', activeLedgerEntry);
  ...
  // Memoizing the filter computation to prevent unneeded heavy arrays re-calculating on simple renders
  const filtered = useMemo(() => {
    const term = search.toLowerCase();
    return records.filter(r => 
      r.entry_type?.toLowerCase().includes(term) || 
      r.atc_id?.toLowerCase().includes(term)
    );
  }, [records, search]);
    ...
    <div className="space-y-4 relative">
      <div className="relative">
        ...
        <input 
          ...
          className="border p-3 pl-10 w-full rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none shadow-sm text-slate-700"
          value={search}
      ...
      <div className="overflow-hidden border border-slate-200 rounded-2xl shadow-sm bg-white">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-[0.15em] border-b border-slate-100">
                  ...
                  <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className="text-sm font-bold text-slate-600">
                        {parseFloat(record.api_mass_mg || 0).toLocaleString()} mg
                      </span>
                    </td>
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-black text-emerald-600">
                          {parseFloat(record.ddd_consumed || 0).toFixed(4)}
                        </span>
      ...
      {activeLedgerEntry && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[90vh] overflow-hidden relative shadow-2xl border border-white/20 flex flex-col">
            ...
            <div className="p-8 overflow-y-auto grow">
              <LedgerForm 
                ...
                atcCodes={atcCodes} 
                companies={companies} 
                companiesCatalog={companiesCatalog} // Pass the new clean list
                initialData={activeLedgerEntry}
                isEdit={true}
                onSuccess={() => {
                  refreshData();
                  setActiveLedgerEntry(null);
                }}
              />
            </div>
