...
import { 
  ...
  ShieldCheck, Loader2, History, Activity, UserPlus, Building2, Gavel
} from 'lucide-react';
...
export default function DeputyDirectorReviewClient({ app, staffList, pdfUrl, loggedInUserId }: DDDReviewProps) {
  ...
  // --- LOGIC PHASE CHECKS ---
  // Phase 3: Technical DD first receives from Director
  const isInitialAssignment = app.currentPoint === 'Technical DD Review';
  
  // Phase 6: Dossier is at the Hub desk
  const isHubStage = app.currentPoint === 'IRSD Hub Clearance';
  
  // Phase 5 return: Technical staff has finished review
  const isTechnicalReturn = app.currentPoint === 'Technical DD Review Return';

  // Combine visibility for the "Endorsement/Forward" box
  ...
  // Determine the relevant division for filtering
  // If we are in the Hub, we filter for IRSD staff. Otherwise, the assigned technical division.
  ...
  const handleAssign = async () => {
    if (!selectedStaffId) return alert(`Select an officer from ${activeAssignmentDivision}.`);
    if (!remarks.trim()) return alert("QMS Requirement: Please provide processing instructions.");
    
    startTransition(async () => {
      // Logic inside assignToStaff (in ddd.ts) will handle point switching 
      // based on whether the actor is IRSD or Technical.
  ...
  const handleEndorse = async () => {
    if (!remarks.trim()) return alert("QMS Requirement: Please provide concurrence/recommendation notes.");
    startTransition(async () => {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 p-8 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="col-span-5 space-y-6 overflow-y-auto max-h-[90vh] pr-2 custom-scrollbar">
        ...
        {/* 2. ASSIGNMENT BOX (Shows for Technical Desk OR IRSD Hub Vetting) */}
          ...
          <div className={`p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden transition-all duration-500 ${isHubStage ? 'bg-emerald-600' : 'bg-blue-600'}`}>
            <div className="relative z-10">
                <h3 className="text-[11px] font-black uppercase tracking-widest mb-6 flex items-center gap-2">
                  ...
                  {isHubStage ? "Delegate to IRSD Vetting Officer" : `Dispatch to ${appDivision} Officer`}
                </h3>
                <div className="space-y-4">
                    <select 
                    ...
                    >
                      <option value="" className="text-slate-900">Select Reviewer from {activeAssignmentDivision}...</option>
                      {staffList
                        .filter(s => s.role === 'Staff' && s.division === activeAssignmentDivision)
                        .map(s => (
                    ...
                    <textarea 
                      ...
                      placeholder={`Enter instructions for ${activeAssignmentDivision} staff...`}
                    ...
                    <button onClick={handleAssign} disabled={isPending} className="w-full py-5 bg-white text-slate-900 rounded-3xl font-black uppercase text-[10px] hover:shadow-2xl transition-all flex items-center justify-center gap-2">
                      {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : `Initiate ${activeAssignmentDivision} Staff Review`}
                    </button>
                </div>
            </div>
        ...
        {/* 3. REVIEW & ENDORSEMENT BOX (Visible for Technical Return OR Hub Direct Clearance) */}
        {isReviewPhase && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Technical CAPAs Display (Only if they exist) */}
            {technicalCapas.length > 0 && (
            ...
            <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 block ml-2">
                {isHubStage ? "Hub Endorsement (Direct)" : `${appDivision} Divisional Deputy Director's Minute`}
              </label>
              <textarea 
                value={remarks}
                onChange={(e) => setRemarks(e.target.value)}
                ...
                placeholder={isHubStage ? "Direct concurrence for the Director..." : "Forwarding notes to IRSD Hub..."}
              />
