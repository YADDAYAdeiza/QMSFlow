...
function SafeTimestamp({ timestamp }: { timestamp?: string }) {
  ...
  if (!mounted || !timestamp) return <span>--:--</span>;
  ...
  return (
    <div className="grid grid-cols-12 gap-6 p-8 bg-slate-50 min-h-screen font-sans">
        ...
        <div className="bg-white p-3 rounded-[3rem] shadow-2xl border border-slate-200 h-[82vh] relative overflow-hidden">
          {iframeSrc ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 rounded-[2.2rem] text-slate-400 gap-3">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-center px-10">The requested URL is missing.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="col-span-5 space-y-6 overflow-y-auto max-h-[90vh] pr-2 custom-scrollbar pb-10">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 flex items-center gap-2">
              <ClipboardList className="w-4 h-4 text-rose-500" /> {showDelta ? "Snapshot: Rejected Findings" : "Compliance Findings"}
            </h3>
            {lastRework && (
              <button 
              ...
              >
                <Diff className="w-3.5 h-3.5" /> {showDelta ? "View Current" : "Compare Changes"}
              </button>
            )}
          </div>
          <div className="space-y-3">
            {(showDelta ? rejectedFindings : findings).map((f: any, idx: number) => (
              <div key={idx} className={`p-4 rounded-2xl border transition-all ${showDelta ? 'bg-amber-50/50 border-amber-200 opacity-80' : 'bg-slate-50 border-slate-100'}`}>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[8px] font-black uppercase text-slate-400 bg-white px-2 py-1 rounded shadow-sm border border-slate-50">{f.system}</span>
                  <span className={`text-[9px] font-black uppercase ${f.severity === 'Critical' ? 'text-rose-600' : 'text-amber-600'}`}>{f.severity}</span>
                </div>
                ...
                return (
                  <div key={idx} className="group relative pl-6 border-l-2 border-slate-100 pb-2">
                    ...
                    <div className={`p-4 rounded-2xl ${isRework ? 'bg-rose-50 border border-rose-100' : 'bg-slate-50'}`}>
                      ...
                      <p className={`text-[11px] italic leading-relaxed ${isRework ? 'text-rose-800' : 'text-slate-600'}`}>
                        "{note?.text}"
                      </p>
                    </div>
                  </div>
        ...
        {isAssignmentPhase && (
          <div className={`p-8 rounded-[2.5rem] shadow-xl text-white transition-all duration-500 ${isHubEntry ? 'bg-emerald-600' : 'bg-blue-600'}`}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowAllStaff(!showAllStaff)} className="text-[8px] font-bold bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full transition-colors">
                {showAllStaff ? "Reset Filter" : "Show All Divisions"}
              </button>
            </div>
            <div className="space-y-4">
              <select value={selectedStaffId} onChange={(e) => setSelectedStaffId(e.target.value)} className="w-full bg-black/20 border border-white/20 p-5 rounded-3xl text-xs font-bold text-white outline-none cursor-pointer appearance-none">
                <option value="" className="text-slate-900">Select {isHubEntry ? 'IRSD' : activeAssignmentDivision} Officer ({filteredStaff.length} available)</option>
                {filteredStaff.map((s: any) => (
                  <option key={s.id} value={s.id} className="text-slate-900">{s.name} {showAllStaff ? `(${s.division})` : ''}</option>
                ))}
              </select>
        ...
        {isReviewPhase && (
          <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl border border-slate-800">
            ...
            <textarea value={endorsementRemarks} onChange={(e) => setEndorsementRemarks(e.target.value)} className="w-full h-32 bg-slate-800 border-none rounded-[2rem] p-6 text-sm mb-6 italic outline-none text-slate-200" placeholder={isTechnicalReturn ? "Notes for Hub Clearance..." : "Final concurrence for the Director..."} />
            <div className="space-y-3">
      ...
      <RejectionModal 
        ...
        currentStaffId={lastAssignedStaffId}
        staffList={staffList} 
        onSuccess={() => { 
