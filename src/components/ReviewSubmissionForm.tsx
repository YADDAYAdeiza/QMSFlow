...
export default function ReviewSubmissionForm({ appId, division, staffId, initialFindings, comments }: Props) {
  ...
  const onFormSubmit = async (data: any) => {
    ...
    setIsPending(true);
    try {
      // ✅ Now passing staffId as the 4th argument for routing logic
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-10">
      ...
      <div className="pt-10 border-t border-slate-100">
        <div className="flex items-center justify-between mb-6 px-1">
          ...
          {comments.length > 0 && (
            <button type="button" onClick={() => setIsHistoryExpanded(!isHistoryExpanded)} className="text-[9px] font-black text-blue-600 flex items-center gap-1 hover:underline">
              {isHistoryExpanded ? <>HIDE FULL HISTORY <ChevronUp className="w-3 h-3" /></> : <>SHOW ALL {comments.length} ENTRIES <ChevronDown className="w-3 h-3" /></>}
            </button>
          )}
        </div>
        ...
        <div className={`relative overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] ${isHistoryExpanded ? 'max-h-[1000px]' : 'max-h-[220px]'}`}>
          ...
          {!isHistoryExpanded && comments.length > 0 && <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-white to-transparent pointer-events-none" />}
        </div>
      </div>
      ...
      <div className="bg-slate-900 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden">
        ...
        <textarea {...register("justification")} placeholder="Provide a final minute addressing the feedback above..." className="w-full h-32 p-6 bg-slate-800 text-white rounded-[2rem] text-sm border-none italic mb-8 outline-none focus:ring-2 focus:ring-blue-500 transition-all" disabled={isPending} />
        <button type="submit" disabled={isPending || !justification?.trim()} className={`w-full py-6 rounded-[2rem] font-black uppercase text-[11px] tracking-[0.2em] flex items-center justify-center gap-3 transition-all ${!justification?.trim() ? 'bg-slate-800 text-slate-600' : 'bg-blue-600 text-white hover:bg-blue-500 shadow-xl'}`}>
          {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Forward Application <Send className="w-4 h-4" /></>}
        </button>
